...
def run_shell(params, quiet=False):
    ...
    env.update({"TRUSTED_ENVIRONMENT": "true"})
    env.update({"SKIP_CLONE_REPORTING": "true"})
...
def run_test(
    ...
    if not directory and not repo_url:
        ...
        raise ValueError(f"Must supply either a directory for the default repo ({GIT_URL}) or a custom repo.\n")
    ...
    if expected_status == status:
        ...
        print(f"❌ Service did not return expected status (got {status}, expected {expected_status}).")
    ...
    if expected_text:
        if expected_text in body:
            ...
            print(f"❌ Service did not return expected content ({expected_text} not in body.\nBody: {body}")
